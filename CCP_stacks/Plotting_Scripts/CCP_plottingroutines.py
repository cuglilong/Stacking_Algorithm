...
#
    def load_latest(self, name='Megavolume', filter='rff2', conversion='EU60', factor=1.):
        line = open('/raid1/mh826/Stacking_Algorithm/CCP_stacks/CCP_volumes/'+name+'_'+filter+'_'+conversion+'_'+str(factor)+'/filenames.dat', 'r').readlines()[-1]
...
#retrieve profile
#
    def retrieve_profile(self, lon, lat, mindepth, maxdepth):
        i = np.argmin([np.abs(i - lon) for i in self.VOL.grid_lon])
        j = np.argmin([np.abs(j - lat) for j in self.VOL.grid_lat])
        d1 = np.argmin(np.abs(self.VOL.grid_depth - mindepth))
        d2 = np.argmin(np.abs(self.VOL.grid_depth - maxdepth))
        return self.VOL.grid_depth[d1:d2], self.VOL.volume[i,j,d1:d2]/ self.VOL.volumeweight[i,j,d1:d2]
